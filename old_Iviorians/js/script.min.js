...
function appendImages(feed) {
	for(var i = 0; i < feed.length; i++) {
		...
		var image = new Image();
		image.src = feed[i].images.standard_resolution.url;

		$(image).appendTo($slide);

		if(feed[i].caption != null) {

			var text = feed[i].caption.text;
			var $caption =	$('<div />', {
				class: 'caption'
			}).html(text);
...
$(function(){
	...
	if(!Modernizr.sessionstorage) {
		window.location.pathname = '/upgrade-your-browser.html';
	}

	// Top NAv scrolling animation
	$('a','nav').on('click',function(event){
		event.preventDefault();
		$('a.current','nav').removeClass('current');
	...
	// Instagram sourced Slideshow
	if(sessionStorage.getItem('feed') != null) {
		feed = JSON.parse(sessionStorage.getItem('feed'));
		appendImages(feed);
	} else {
		var access_token = '1356075.1fb234f.cc019a0e565f4ca09723b536e530baa1';

		var instagramURL = "https://api.instagram.com/v1/tags/rockawayreliefnow/media/recent?access_token=" + access_token;
		$.ajax({
			type: "GET",
			dataType: "jsonp",
			cache: false,
	...
	$('div.slide','#slideshow').first().addClass('current');
...
function slideshow() {
    ...
    window.setInterval(function () {
    	console.log('called');
    	var $current = $('.current','#slideshow').removeClass('current');

    	var $new_current = ($current.next().length > 0) ? $current.next() : $('div.slide','#slideshow').first();

    	$new_current.addClass('current');

    	var left = ($('div.slide.current','#slideshow').position().left) * -1;

    	$('#slideshow').css('-webkit-transform', 'translate3d(' + left + 'px, 0, 0)');
